...
ユーザー画面は `http://localhost:3000/`、管理画面は `http://localhost:3000/admin` です。ユーザー画面では登録・ログイン、パック詳細、1/10/全口抽選、演出、カード一括還元、配送先登録、発送申請、ポイント履歴を操作できます。両画面はログイン方式・セッショントークン・ナビゲーションを分離しています。

開発用の一般ユーザーは `demo@example.com` / `demo-user-password`（初期10,000pt）で、ユーザー画面から2FAなしでログインできます。デモ管理者は `admin@example.com` / `admin-dev-password` で、管理画面からTOTPを併用します。本番では必ず認証基盤と管理者資格情報を置き換えてください。
